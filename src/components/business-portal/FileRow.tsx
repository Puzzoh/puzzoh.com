...
const FileRow = ({ id, title, description, file, createDate }: FileTypes) => {
  ...
  return (
      <tr>
          ...
          <div className="flex items-center space-x-3">
            <div className="avatar">
              <div className="mask mask-squircle w-12 h-12">
                <img src="https://upload.wikimedia.org/wikipedia/commons/thumb/8/87/PDF_file_icon.svg/1200px-PDF_file_icon.svg.png" alt="file type" />
              </div>
            </div>
            <div>
        ...
        <th>
          <Popup trigger={<button className="btn btn-ghost btn-xs">DELETE</button>}>
            <p> Are you sure you want to delete {title}? </p>
            <button className="btn btn-ghost btn-xs">Cancel</button>
            <button className="btn btn-warning btn-xs" onClick={deleteFile}>Confirm</button>
          </Popup>
        </th>
      </tr>
  )
