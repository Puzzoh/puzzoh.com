...
const NewVoucher = () => {
  ...
  const createNewVoucher = async (e: React.ChangeEvent<any>) => {
    ...
    await setVoucherInputs(voucherInputs => {
      return {
      ...voucherInputs, category: selected.name, expireDate: chosenDate }
    });
  ...
  return (
      <form className="flex flex-col space-y-6 border-black border-solid border-1" encType="multipart/form-data"
      onSubmit={createNewVoucher}>
            ...
            <div>
              ...
              <input type="range" min="0" max="100" className="range max-w-xs" step="5"
                value={discount}
            ...
            <div>
              <h2>
                Images related to voucher (up to 3)
              </h2>
              <input type="file" multiple accept="image/*, application/pdf"
                    className="file-input file-input-bordered w-full max-w-xl"
                    name="image" onChange={handleFileChange} />
              <span className='text-sm italic'> (Please upload in PDF or image format) </span>
            </div>
            ...
            <div>
              ...
              />
              <span className='text-sm italic'> (The voucher will be marked as `non-expiring` if no date is chosen) </span>
            </div>
